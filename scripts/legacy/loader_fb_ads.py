...
def load_excel(path: Path = INPUT_FILE) -> pd.DataFrame:
    """Load file (Excel or CSV) based on file extension."""
...
def filter_relevant(df: pd.DataFrame, input_path: Path = INPUT_FILE) -> pd.DataFrame:
    """Filter relevant pages based on ad category and keywords."""
...
def load_and_process(input_path: Path = INPUT_FILE, output_path: Path = OUTPUT_FILE) -> pd.DataFrame:
    ...
    df = deduplicate(df)
    df = filter_relevant(df, input_path)
